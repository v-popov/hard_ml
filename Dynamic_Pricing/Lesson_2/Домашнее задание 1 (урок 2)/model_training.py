...
x_train = joblib.load("""C:/Users/victor/Google Drive/files/Courses/Hardcore_ML/Динамическое Ценообразование/Урок 2/Домашнее задание 1 (урок 2)/x_train.j""")
# x_train.drop(columns=['mosque', 'prom_part', 'month', 'healthcare_facilities', 'sub_area',
...
y_train = joblib.load('C:/Users/victor/Google Drive/files/Courses/Hardcore_ML/Динамическое Ценообразование/Урок 2/Домашнее задание 1 (урок 2)/y_train.j').values
...
def objective(space):
    clf = xgb.XGBRegressor(objective='reg:squarederror', n_jobs=15, n_estimators=500, seed=1, max_depth=int(space['max_depth']), gamma=space['gamma'],
...
def test(x_train, y_train):
    params = {'colsample_bytree': 0.6831091101258099, 'gamma': 3.4889550120515027, 'max_depth': 5.0, 'min_child_weight': 7.0, 'reg_alpha': 123.0, 'reg_lambda': 0.5398957743166721, 'subsample': 0.7748432218622309}
    for int_param_name in ['max_depth', 'min_child_weight', 'reg_alpha']:
        params[int_param_name] = int(params[int_param_name])
    cv = ShuffleSplit(n_splits=5, test_size=0.3, random_state=1)
    mse_scores = []
    for train_inds, test_inds in cv.split(x_train):
        if params:
            model = xgb.XGBRegressor(objective='reg:squarederror', n_jobs=15, n_estimators=500, seed=1, **params)
        else:
            model = xgb.XGBRegressor(objective='reg:squarederror', n_jobs=15, n_estimators=500, seed=1, subsample=1.0, min_child_weight=1.6361503076509394, max_depth=3, gamma=1.0774387353144212, colsample_bytree=0.6)
    ...
    joblib.dump(model, 'C:/Users/victor/Google Drive/files/Courses/Hardcore_ML/Динамическое Ценообразование/Урок 2/Домашнее задание 1 (урок 2)/model.j')
...
if __name__ == '__main__':
    ...
    # if not IS_RUN_CV:
    #     test(x_train.values, y_train.values)
