...
df_sales = df_sales.merge(df_users[['user_id', 'stratum']], on='user_id', how='left')
...
def linearize(df, kappa=None):
    aggregation = df.groupby(['user_id', 'day', 'stratum']).agg(x=('sales', 'sum'),
                                                     y=('sales', 'count'))
    if kappa is None:
...
def __check_test(user_a, user_b):
...
def _check_test(test):
    ...
    # AA
    if __check_test(group_a_one, group_a_two) < 0.05:
        return False

    return __check_test(group_a_one + group_a_two, group_b) < 0.05
