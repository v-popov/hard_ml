...
days_range = np.arange(49, 56)
unique_user_ids = df_users['user_id'].unique()
df_all_users = pd.DataFrame({'user_id': np.repeat(unique_user_ids, len(days_range)),
                             'day': np.tile(days_range, len(unique_user_ids))})
...
df_sales = df_all_users.merge(df_sales, on=['user_id', 'day'], how='left')
df_sales['sales'].fillna(0., inplace=True)
