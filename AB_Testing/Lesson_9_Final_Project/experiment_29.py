...
# outliers removal
quantile = np.percentile(df_sales['sales'], 99.9)
df_sales = df_sales[df_sales['sales'] < quantile]

# dedup
dupl_inds = df_sales.duplicated(subset=None, keep='first')
df_sales = df_sales[~dupl_inds]

df_sales_cov = df_sales[df_sales['day'].isin(np.arange(35, 42))]
df_sales_cov['day'] += 14
df_sales_cov = df_sales_cov.groupby(['user_id', 'day'])['sales'].sum().reset_index()
