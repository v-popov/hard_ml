...
# speed up?
def calc_strat_mean_var(df, strat_column, target_name):
    groupped = df.groupby(strat_column)[target_name]
    return (groupped.mean() * weights).sum(), (groupped.var() * weights).sum()
