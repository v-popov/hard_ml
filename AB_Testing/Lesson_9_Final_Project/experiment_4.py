...
def _check_test(test):
    ...
    # AA test
    sales_a_one = df_sales[df_sales['user_id'].isin(group_a_one)]['sales'].values
    sales_a_two = df_sales[df_sales['user_id'].isin(group_a_two)]['sales'].values
    if ttest_ind(sales_a_one, sales_a_two)[1] < 0.05:
        return False
