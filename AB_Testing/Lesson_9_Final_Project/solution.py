...
df_sales_cov['day'] += 14
df_sales_cov = df_sales_cov.groupby(['user_id']).agg(sales_cov=('sales', 'sum'),
                                                     y_cov=('sales', 'count')).reset_index()

# эксперимент проводился с 49 до 55 день включительно
df_sales = df_sales[df_sales['day'].isin(np.arange(49, 56))]
df_sales = df_sales.groupby(['user_id']).agg(sales=('sales', 'sum'),
                                             y=('sales', 'count')).reset_index()
...
def linearize(df, suffix='', kappa=None):
    if kappa is None:
        kappa = df[f'sales{suffix}'].sum() / df[f'y{suffix}'].sum()

    df[f'sales{suffix}'] = df[f'sales{suffix}'] - kappa * df[f'y{suffix}']
...
def test_groups(sub_df_a, sub_df_b):
    # sub_df_a, kappa = linearize(sub_df_a)
    # sub_df_b, _ = linearize(sub_df_b, kappa=kappa)
    #
    # sub_df_a, _ = linearize(sub_df_a, suffix='_cov', kappa=kappa)
    # sub_df_b, _ = linearize(sub_df_b, suffix='_cov', kappa=kappa)

    sub_df_a['sales'] = sub_df_a['sales'] - theta * sub_df_a['sales_cov']
    sub_df_b['sales'] = sub_df_b['sales'] - theta * sub_df_b['sales_cov']
