...
def select_stratified_groups(data, strat_columns, group_size, weights=None, seed=None):
    ...
    orig_types = data.dtypes.to_dict()
    result = None
    ...
    for i, k in enumerate(weights):
        ...
        if i == len(weights)-1 and sizes_count != group_size * 2:
            # print(f'group_size: {group_size}; sizes_count: {sizes_count}; curr strat_size_both: {strat_size_both}')
            strat_size_both += group_size * 2 - sizes_count
            # print(f'new strat_size_both: {strat_size_both}')
    ...
    print(pilot[orig_cols].shape, control[orig_cols].shape)
    return (pilot[orig_cols].astype(orig_types), control[orig_cols].astype(orig_types))